...
function EticaPage() {
    ...
    const imagenes = [
        require('../assets/imagenes/organi.png'),
        require('../assets/imagenes/integrantes.png')
    ];
    const [indice, setIndice] = useState(0);

    const siguienteImagen = () => setIndice((indice + 1) % imagenes.length);
    const anteriorImagen = () => setIndice((indice - 1 + imagenes.length) % imagenes.length);
    ...
    return (
        <div className="etica-container">
            ...
            {/* Carrusel de imágenes */}
            <div className="etica-carrusel">
                <button className="etica-carrusel-btn" onClick={anteriorImagen}>&lt;</button>
                <img
                    src={imagenes[indice]}
                    alt={`Imagen ${indice + 1}`}
                    className="etica-carrusel-img"
                />
                <button className="etica-carrusel-btn" onClick={siguienteImagen}>&gt;</button>
            </div>
        </div>
    );
}
